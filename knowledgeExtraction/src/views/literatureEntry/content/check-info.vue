  ...
  <div class="content">
    <div class="wrap">
        ...
        <div style="height: 30px">
          <!-- <span
            style="
              ...
              text-align: center;
            "
            ><span style="color: #82848a">灰色未处理，</span
            ><span style="color: #ff4949; margin: 0 2px">红色处理中，</span
            ><span style="color: #67c23a">绿色已完成</span></span
          > -->
          <!-- <span>跳转</span>
          <el-input
            v-model="num"
      ...
      <div style="width: 50%">
        <div class="words_form">
          ...
          <span class="ml-20px">
            ...
            <el-switch
              ...
              :active-text="highlightMode ? '标注文本' : '清晰文本'"
              ml="10px"
            ></el-switch>
          </span>
          <span class="float-right">
            <el-button type="primary" @click="save">保存</el-button>
            <el-button type="success" @click="doAudit">审核通过</el-button>
        ...
        <div
          :class="['slider-box text_format', {'windows' : windows}, {'mac': mac}]"
          :style="`font-size: ${sizeNum}px;height: calc(100% - ${
            temp1.auditor && temp1.auditor.length ? 80 : 50
          }px)`"
        >
          <editor
            v-if="highlightMode"
            v-model="temp1.audit_words"
            :color="userColor"
            :disabled="false"
          />
          <div v-else v-html="temp1.words" class="py-10px" style="width:100vw"></div>
        </div>
      </div>
    </div>
    <el-dialog
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      console.log(this.$refs["img-" + this.num][0])
      this.$refs["img-" + this.num][0].scrollIntoView();
      this.review(this.list[this.num - 1]);
    },
    //目录列表
    catalogueList() {
    ...
    save() {
      ...
      const reg1 = /(<\/?mark.*?>)/gi
      const reg2 = /<s[^>]*>(.|\n)*<\/s>/gi;
      const clearWords = rootDom.innerHTML.replace(reg1, '').replace(reg2, '')
      updateVision({
        ...this.bidobj,
        ...
        getVisionList(this.bidobj).then((res) => {
          this.list = res.result.version;
          this.review(this.list[this.page - 1]);
        });
      });
    },
    // 放大
