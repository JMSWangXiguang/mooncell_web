<template>
  <div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :span="16">
        <div class="right-box slider-box scrollbar-thin">
          <ul>
            <li
            ...
            >
              ...
              <span class="flex-2" @click.stop="handleEdit(item)">{{ item.title }}</span>
              <i class="el-icon-delete remove-btn" @click.stop="remove(index)" />
            </li>
          </ul>
        </div>
      </el-col>
...
<script>
import { referenceTree, getReference, updateReference } from '@/api/knowledge/disease'
import literatrueDialog from './literatrueDialog'
import { each, uniqWith, cloneDeep, assign, map } from 'lodash'
export default {
  name: 'TreeBox',
  components: {
    literatrueDialog,
  ...
  methods: {
    ...
    getReference(paramsObj) {
      ...
      getReference(paramsObj).then((res) => {
        ...
        this.dataList = assign(this.objProperty.references)
      })
    },
    // 过滤树节点
    ...
    // },
    append(data) {
      let list = cloneDeep(this.dataList)
      const findDoc = (arr) =>
        each(arr, (it) => {
          if (it.docid) {
            list.push(it)
      ...
      list = uniqWith(list, (a, b) => a.docid === b.docid)
      this.dataList = list
    },
    //移除症状
    remove(index) {
    ...
    saveDate() {
      updateReference({
        ...this.objProperty,
        references: map(this.dataList, ({ docid }) => ({ docid })),
      }).then((res) => {
        if (res.status == 200) {
          this.closeDialog()
