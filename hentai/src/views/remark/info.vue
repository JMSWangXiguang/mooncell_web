 ...
 * @LastEditors: imali
 * @LastEditTime: 2022-05-06 16:29:34
-->
<template>
  <div
    ref="settag"
  ...
  >
    <el-row :gutter="20">
      <el-col :span="12">
        <div
          ...
          "
        >
          <span class="mr-10px">{{ form.category_title }}</span>
          <span class="mr-10px">{{ form.corpus_id }}</span>
          <span class="mr-5px">标注人：{{ form.marker }}</span>
        </div>
      </el-col>
      <el-col :span="12" style="text-align: right">
        ...
        <el-button-group style="padding-left: 20px">
          ...
          <!-- <el-button type="danger" size="mini" icon="el-icon-delete" @click="deleteWork">跳过文本</el-button> -->
          <el-button size="mini" @click="$router.push({ path: 'remark' })">返回</el-button>
    ...
    <svg
      width="98%"
      style="position: absolute; height: 90%; top: 50px; bottom: 0"
      :height="svgheight"
                ...
                :style="
                  model !== '原始'
                    ? `border: 1px ${item.color} solid;` +
                      (!item.rubi
                        ? `background:${
                            item.key == chosekey ? item.color : item.color
                          };color:#ffffff`
...
<script>
import { getFrontWork, submitWork } from '@/api/simple'
import { getWork } from '@/api/remark'
...
export default {
  ...
  created() {
    ...
    this.corpusId = this.form.corpus_id
    this.marker_id = this.form.marker_id
    this.category_title = this.form.category_title
  ...
  methods: {
    ...
    getWork() {
      this.loading = true
      this.model = '词性'
      getWork({ corpus_id: this.corpusId, marker_id: this.marker_id })
        .then((res) => {
          this.links = []
          this.corpus = []
          ...
          // this.corpus.filter((item) => item.link.length > 0).map((item) => { return { from: item.key, tolinks: item.link } }).forEach((item) => {
          ...
          // })
          const links = []
          const linksMap = this.corpus.reduce((pre, item) => {
            if (!item.link || !item.link.length) return pre
            if (!pre[item.key]) pre[item.key] = []

            item.link.forEach((link) => {
              if (!pre[link] || !pre[link].includes(item.key)) {
                pre[item.key].push(link)
                links.push({
                  from: item.key,
                  to: link,
                })
    ...
    clearspan(item) {
      ...
      var corpu = this.corpus.find((i) => i.key === item.key)
      console.log(corpu.speech, 789)
      corpu.speech = ''
    ...
    getLineByPoint(point1, point2, color = '') {
      ...
      if (point1.top === point2.top) {
        ...
        if (Math.abs(point1.xpoint - point2.xpoint) < 100) {
          ...
          // 起点在终点的左上方
          const points = deepClone({
            xpoint: point1.xpoint,
            top: point1.top + 35,
          })
          const pointe = deepClone({
