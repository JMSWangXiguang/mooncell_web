          ...
          <div class="content">
            <template v-for="(item, index) in corpus1">
              <span
                ...
                @contextmenu.prevent="clearspan(item)"
                @dblclick="openMenu(item, $event)"
                v-html="item.txt"
              />
              <span
                v-else
...
export default {
    ...
    itemLinks() {
      if (this.model === '关系') {
        if (this.selectedItem) {
          ...
          const links = this.links.filter((item) => item.from === id || item.to === id)
          const corpus1 = this.corpus1.filter(
  ...
  methods: {
    // // 监听移入到待划分词span
    // mouseOverEmptySpan(item) {
    //   console.log('移入',item)
    //   this.waitSplitSpan = item
    // },
    // // 监听移除待划分词span
    // mouseLeaveEmptySpan(item) {
    //   console.log('移出',item)
    //   this.waitSplitSpan = undefined
    // },
    // 双击事件:解除词性、解除关联
    clearspan(item) {
